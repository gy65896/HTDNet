...
def dTrainSynRGB(img_filepath, depth_filepath, patch_size, stride):
	...
	with h5py.File(train_haze, 'w') as h5f:
		for i in range(len(img_files)):
			...
			o_img = cv2.imread(img_filepath + '/' + filename + '.jpg')
			o_img = cv2.resize(o_img,(360,360))
			o_depth = np.array(h5py.File(depth_filepath + '/' + filename + '.mat')['depth'])
			#img= samesize(img,(360,360))
			for sca in scales:
				img = o_img
				depth = o_depth
				img = cv2.resize(o_img, (0, 0), fx=sca, fy=sca, interpolation=cv2.INTER_CUBIC)
				depth = cv2.resize(o_depth, (0, 0), fx=sca, fy=sca, interpolation=cv2.INTER_CUBIC)
				print(img.shape)
