    ...
    if argspar.syn:
        ...
        for i in range(len(files_clear)):   
            ...
            with torch.no_grad():
                ...
                haze, clear = syn_haze(clear, 'test', 0.7, 0.7, 0.2, 0.2)
                #模型处理
                starttime = time.clock()
                Kout,Eout,Fout = model(haze) 
                endtime1 = time.clock()
                
                print('The ' + name+' Time:' +str(endtime1-starttime)+'s.'+\
                      'PSNR:%.4f'%(tensor_psnr(clear, Fout)))
                temp = torch.cat((clear, haze, Kout,Eout,Fout), dim = 3)
        ...
        for i in range(len(files_low)):   
            ...
            with torch.no_grad():
                name = files_low[i][:-4]
                haze = cv2.imread(argspar.input_testr + '/' + files_low[i])
                ...
                minF = torch.min(Fout)
                maxF = torch.max(Fout)
                Fout = (Fout-minF)/(maxF-minF)
                endtime1 = time.clock()
                #Result2 = R_out*(1-L_out)+L_out*low
                
                print('The ' + name+' Time:' +str(endtime1-starttime)+'s.')
                temp = torch.cat((haze, Kout,Eout,Fout), dim = 3)
                temp = tensor2img(temp)
                if (epoch+1):
...
def train(model, optimizer, cur_epoch, argspar, loader_train_syn):
    ...
    for epoch in range(cur_epoch, argspar.epoch):#批次数
        ...
        for i,data in enumerate(loader_train_syn,0):
            #合成图像
            low, clear = syn_haze(data,'train', 0.7, 0.9, 0.2, 0.6)
            #模型输出
            Kout,Eout,Fout = model(low)
            Kloss = L2_loss(Kout,clear)
            Eloss = L2_loss(Eout,clear)

            loss = 0.5*(L2_loss(Kout,clear)+L2_loss(Eout,clear))+0.10*L2_loss(Eout,Kout)+0.6*L2_loss(Fout,clear)
            #0.5*((1-Kloss/(Kloss+Eloss))*L2_loss(Kout,clear) +\
               #(1-Eloss/(Kloss+Eloss))*L2_loss(Eout,clear)) + \
                #0.10*L2_loss(Eout,Kout) + 0.25*(L2_loss(Fout,clear)+ssim_loss(Fout,clear))
            #计算损失函数
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            ...
            print("[epoch %d][%d/%d] lr :%f loss: %.4f PSNR: %.6f"%(epoch+1, i+1, \
                len(loader_train_syn), learnrate, loss.item(),tensor_psnr(Fout, clear)))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--epoch", type=int, default = 1000, help = '总批次数')
    parser.add_argument("--batch_size", type=str, default =8, help = '批次大小')
